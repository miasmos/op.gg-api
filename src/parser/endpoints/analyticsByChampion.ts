...
class AnalyticsByChampion extends Endpoint {
    path() {
    ...
    parse($) {
        ...
        data.skills.pickrate = parseFloat(
            $(".ChampionStatsSummaryTable .Row:nth-child(1) .PickRate")
                .text()
                .replace("Pick Rate", "")
        );
        data.skills.pickrateSample = parseInt(
            $(".ChampionStatsSummaryTable .Row:nth-child(1) .PickRate span").text().replace(",", "")
        );
        data.skills.winrate = parseFloat(
            $(".ChampionStatsSummaryTable .Row:nth-child(1) .WinRate")
                .text()
                .replace("Win Rate", "")
        );
        //end parse skills
        ...
        var rowCount = parseInt(
            $(".ChampionStatsSummaryTable .Row:nth-child(2) .Label").attr("rowspan")
        );
        for (var i = rowStart; i < rowStart + rowCount; i++) {
            ...
            spellContainer.pickrateSample = parseInt(
                element.find(".PickRate span").text().replace(",", "")
            );
        ...
        data.starterItems = [];
        rowCount = parseInt(
            $(`.ChampionStatsSummaryTable .Row:nth-child(${rowStart}) .Label`).attr("rowspan")
        );
        for (var i = rowStart; i < rowStart + rowCount; i++) {
            var element = $(".ChampionStatsSummaryTable .Row").eq(i),
                itemContainer = {},
            ...
            itemContainer.pickrateSample = parseInt(
                element.find(".PickRate span").text().replace(",", "")
            );
        ...
        for (var i = rowStart; i < rowStart + rowCount; i++) {
            ...
            masteryContainer.pickrateSample = parseInt(
                element.find(".PickRate span").text().replace(",", "")
            );
        ...
        for (var i = rowStart; i < rowStart + rowCount; i++) {
            ...
            runesContainer.pickrateSample = parseInt(
                element.find(".PickRate span").text().replace(",", "")
            );
        ...
        for (var index in graphtypes) {
            ...
            switch (
                graphtype //the below regex is invalid when inserting a variable for some reason, let's just do it the long way
            ) {
                case "WinRate":
                    regex = new RegExp(
                        /GenerateGraph\('WinRateGraph',\s*\d*,\s*\d*,\s?\[.*?\],(.*?])/g
                    ).exec($.html());
                    break;
                case "PickRate":
                    regex = new RegExp(
                        /GenerateGraph\('PickRateGraph',\s*\d*,\s*\d*,\s?\[.*?\],(.*?])/g
                    ).exec($.html());
                    break;
                case "GameLengthWinRate":
                    regex = new RegExp(
                        /GenerateGraph\('GameLengthWinRateGraph',\s*\d*,\s*\d*,\s?\[.*?\],(.*?])/g
                    ).exec($.html());
                    break;
            }
            ...
            if (!!regex && regex.length) {
                ...
                try {
                    ...
                    for (var item in data1) {
                        ...
                        data1[item].patch = parseFloat(data1[item].patchIndex)
                            ? parseFloat(data1[item].patchIndex)
                            : undefined;
                        delete data1[item]["patchIndex"];

                        data1[item].date = data1[item].patchDate
                            ? data1[item].patchDate
                            : undefined;
                        delete data1[item]["patchDate"];

                        data1[item].rank = parseInt(
                            data1[item].rank.slice(0, data1[item].rank.length - 2)
                        );
                    }
...
export default AnalyticsByChampion;
