...
class MatchesByTimestamp extends Endpoint {
    path() {
    ...
    parse($, json) {
        ...
        $(".GameItemWrap").each((i, game) => {
            ...
            $(".FollowPlayers .Team .Summoner").each((j, _player) => {
                ...
                player.championImage = this.Strip(
                    $(".ChampionImage .Image").css("background-image")
                );
        ...
        return data.length ? data : Error(errorMessages.NO_RESULTS, responseCodes.NO_RESULTS);
    }
}

export default MatchesByTimestamp;
