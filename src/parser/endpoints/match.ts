    ...
    parse($, json) {
        ...
        $(".GameDetailTable").each((key0, value0) => {
            ...
            $(".Content > .Row").each((key, value) => {
                ...
                player.killParticipation = parseFloat(
                    $(".CKRate").text().replace("(", "").replace("%)", "")
                );
            ...
            team.result =
                $(".HeaderCell").eq(0).find(".GameResult").text().indexOf("Victory") > -1
                    ? "Victory"
                    : "Defeat";
            team.color =
                $(".Header .HeaderCell").eq(0).text().indexOf("Blue") > -1 ? "Blue" : "Red";
