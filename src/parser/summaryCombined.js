...
class SummaryCombined extends Endpoint {
    path() {
        return "/summoner/";
    }
    ...
    parse($, json) {
        ...
        if (temp && temp.length > 0 && temp[2]) {
            ...
            recent.kdaRatio = parseFloat(
                $(".GameAverageStats .KDARatio .KDARatio").text().slice(0, -2)
            );
            recent.killParticipation = parseInt(
                $(".GameAverageStats .KDARatio .CKRate span").text().replace("%", "")
            );
            ...
            $(".GameItemWrap").each((i, game) => {
                ...
                game.spell1Image = this.Strip(
                    $(".SummonerSpell .Spell:first-child img").attr("src")
                );
                game.spell2 = this.Strip($(".SummonerSpell .Spell:last-child img").attr("alt"));
                game.spell2Image = this.Strip(
                    $(".SummonerSpell .Spell:last-child img").attr("src")
                );
                ...
                game.killParticipation = parseInt(
                    this.Strip($(".CKRate").text().replace("P/Kill ", ""))
                );
