...
module.exports = class Endpoint {
	...
	Uri(params) {
		...
		var uri = this.baseURI.replace('region', params.region) + this.Path(),
			didHaveParams = false
		...
		for (var i in params) {
			if (params.length == 1 && typeof params[i] === 'undefined') break
			if (i == 0 && !didHaveParams) uri += '?'
			if (i !== 'region' && typeof params[i] !== 'undefined') uri += i + '=' + params[i] + '&'
		}
		return uri
	}
