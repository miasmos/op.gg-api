...
module.exports = class AnalyticsByChampionEndpoint extends Endpoint {
	...
	Parse($) {
		...
		data.role = this.Strip($('.ChampionRole .Role .Label').text()).toLowerCase()
		data.rolePercent = parseFloat($('.ChampionRole .Role .Value').text())
		data.winrateRank = $('.ChampionRoleList .Stats').eq(0).find('.Value').attr('title')
		regex = new RegExp(/^[0-9]{0,4}/g).exec(data.winrateRank)
		data.winrateRank = !!regex && regex.length ? parseInt(regex[0]) : undefined
		data.winrate = parseFloat($('.ChampionRoleList .Stats').eq(0).find('small').text())
		data.pickrateRank = $('.ChampionRoleList .Stats').eq(1).find('.Value').attr('title')
		regex = new RegExp(/^[0-9]{0,4}/g).exec(data.pickrateRank)
		data.pickrateRank = !!regex && regex.length ? parseInt(regex[0]) : undefined
		data.pickrate = parseFloat($('.ChampionRoleList .Stats').eq(1).find('small').text())
